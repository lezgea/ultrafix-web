...
export const DaySelect: React.FC<IDaySelectProps> = (props) => {
    let { selected, day, weekDay, onSelect } = props;
    ...
    return (
        ...
        >
            <p className={`text-gray-700 text-xl leading-lg ${selected ? 'text-white' : 'group-hover:text-primaryDark'}`}>{day}</p>
            <span className={`text-gray-500 font-regular ${selected ? 'text-white' : 'group-hover:text-primaryDark'}`}>{weekDay.substring(0, 3)}</span>
        </div>
    )
}